...
- [x] 대기 중에는 `~/.Trash`, `~/Library/Caches`, `~/Library/Logs`를 순회하지 않는다.
- [x] **정크 파일 정리…** 를 골랐을 때만 세 위치를 살펴본다.
...
- [x] 정크 정리를 고르기 전에는 세 폴더를 살펴보지 않는다.
- [x] 정리를 고르면 지워도 되는 용량만 확인 창에 보이고, 취소하면 파일이 지워지지 않는다.
