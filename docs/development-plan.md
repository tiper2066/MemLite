...
상세 항목 아래 구분선을 두고 **정크 파일 정리…** 와 **로그인 시 열기**를 둔다. 그 아래 다시 구분선을 두고 눌리지 않는 **MemLite 1.0**과 **종료**를 둔다. Dock 아이콘이 없으므로 종료는 메뉴에만 있다.
...
- `StatusItemController`는 스냅샷을 메뉴 막대 문구와 메뉴 항목으로 그리고, 정크 정리 확인 창과 로그인 시 열기를 다룬다.
...
5. 클릭 메뉴에 상세 7항목, 정크 파일 정리, 로그인 시 열기, 버전, 종료를 넣는다.
