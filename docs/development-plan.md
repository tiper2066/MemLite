...
MemLite는 macOS 상단 메뉴 막대에서 메모리 사용량을 보여주는 개인용 앱이다. 활성상태보기와 같은 메모리 항목을 읽되, 상주 창은 두지 않는다. 메뉴 막대에는 짧은 숫자와 단계 색만 표시하고, 클릭하면 상세 항목을 메뉴로 펼친다. 같은 메뉴에서 정크 파일 정리를 고를 수 있다. 개발이 끝나면 다른 Mac 앱처럼 DMG로 설치한다.
...
- 클릭 한 번으로 활성상태보기의 메모리 상세 항목을 본다.
- 메뉴에서 고를 때만 지워도 되는 정크 파일 용량을 보여 주고, 확인 후 그것만 지운다.
- 완료된 앱을 DMG로 열어 응용 프로그램 폴더에 넣어 설치한다.

### 포함

- 메뉴 막대 텍스트 (`19.7 / 24 GB`)
- 사용량 숫자의 단계 색
- 클릭 메뉴의 상세 수치
...
- App Store 배포, 자동 업데이트, 로그인 항목 등록 UI
...
글자색은 AppKit `NSStatusItem` 버튼의 `attributedTitle`로 넣는다. SwiftUI `MenuBarExtra` 레이블은 메뉴 막대에서 색이 빠질 수 있으므로 사용하지 않는다.
...
상세 항목 아래 구분선을 두고 **정크 파일 정리…** 를 둔다. 그 아래 다시 구분선을 두고 **종료**를 둔다. Dock 아이콘이 없으므로 종료는 메뉴에만 있다.

### 정크 파일 정리

대기 중에는 폴더를 살펴보지 않는다. **정크 파일 정리…** 를 골랐을 때만 아래 세 위치의 용량을 계산한다.
...
- `JunkCleaner`는 메뉴에서 요청이 있을 때만 세 폴더를 보고, 지워도 되는 파일의 용량만 계산한 뒤 그 파일만 지운다.
- `StatusItemController`는 스냅샷을 메뉴 막대 문구와 메뉴 항목으로 그리고, 정크 정리 확인 창을 띄운다.
...
- 정크 파일 용량 계산과 삭제는 2초 주기에 넣지 않는다. 메뉴 항목을 골랐을 때만 세 폴더를 순회한다.
...
5. 클릭 메뉴에 상세 7항목, 정크 파일 정리, 종료를 넣는다.
...
- 스왑이 없으면 `0바이트`로 보인다.
- 정크 파일 정리를 고르기 전에는 휴지통과 캐시, 로그를 살펴보지 않는다.
- 정리를 고르면 지워도 되는 용량만 항목별과 합계로 확인 창에 보이고, 취소하면 파일이 지워지지 않는다.
